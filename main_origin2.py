...
class Data:
    ...
    def checkData(self):
        ...
        sql = '''SELECT IFNULL(MAX(index_num),0) FROM test_table WHERE create_date = %s'''
    ...
    def selectData(self):
        ...
        sql = '''SELECT id, index_num, model_name, state FROM test_table WHERE create_date = %s'''
        now = datetime.datetime.now()
        date = now.strftime('%Y-%m-%d')
        curs.execute(sql, date)
        rows = curs.fetchall()
    ...
    def clearItem(self):
        ...
        sql = '''DELETE FROM test_table WHERE create_date = %s AND state = 0'''
        now = datetime.datetime.now()
        date = now.strftime('%Y-%m-%d')
        try :
            curs.execute(sql, date)
    ...
    def insertItem(self, num, model):
        ...
        sql = '''INSERT INTO test_table (index_num, model_name, create_date) VALUES (%s, %s, %s)'''
        now = datetime.datetime.now()
        date = now.strftime('%Y-%m-%d')
    ...
    def deleteItem(self, model_id):
        ...
        if not curs.execute(sql, (int(model_id))):
            ...
            if int(last_id) != int(model_id):
                sql = '''UPDATE test_table SET index_num = index_num-1 where id > %s'''
                if not curs.execute(sql, (int(model_id))):
                    flash("처리 오류[-2000]")
